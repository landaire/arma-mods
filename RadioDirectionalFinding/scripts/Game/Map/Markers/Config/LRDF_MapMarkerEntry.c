...
class LRDF_MapMarkerEntry: SCR_MapMarkerEntryDynamic
{
	...
	protected ref map<int, LRDF_MapMarker> m_mPlayerMarkers = new map<int, LRDF_MapMarker>();
	
	//------------------------------------------------------------------------------------------------
	//! Register marker here so it can be fetched from the map
	void RegisterMarker(LRDF_MapMarker marker, int playerId)
	{
	...
	void TryCreateOrUpdateMarker(SCR_AIGroup group, int playerId) {
		...
		LRDF_MapMarker marker = m_mPlayerMarkers.Get(playerId);
		...
		if (!marker) {
			...
			marker = LRDF_MapMarker.Cast(m_MarkerMgr.InsertDynamicMarker(SCR_EMapMarkerType.LRDF_TEAM_MARKER, player));
			if (!marker)
				return;
	...
	protected void OnPlayerKilledClient( notnull SCR_InstigatorContextData instigatorContextData)
	{
		...
		for (int i = 0; i < playerIds.Count(); i++) {
			LRDF_MapMarker marker = m_mPlayerMarkers.Get(playerIds[i]);
			if (marker) {
	...
	protected void OnPlayerSpawnedClient( int playerId, IEntity playerEntity)
	{
		...
		for (int i = 0; i < m_mPlayerMarkers.Count(); i++) {
			...
			LRDF_MapMarker marker = m_mPlayerMarkers.Get(key);
			if (marker) {
				marker.RefreshPositionCondition();
			}
		}
	...
	protected void OnGroupCustomNameChanged(SCR_AIGroup group)
	{
		...
		for (int i = 0; i < groupMembers.Count(); i++) {
			LRDF_MapMarker marker = m_mPlayerMarkers.Get(groupMembers[i]);
			if (marker)
	...
	{
	 	return SCR_EMapMarkerType.LRDF_TEAM_MARKER;
	}
